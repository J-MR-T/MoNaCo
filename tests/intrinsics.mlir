...
module{
  llvm.func @main() -> i32{
    ...
    %isvolatile = llvm.mlir.constant(0 : i1) : i1
    "llvm.intr.memcpy"(%4, %3, %len, %isvolatile) : (!llvm.ptr, !llvm.ptr, i32, i1) -> () // args seem to be dst, src, len, isVolatile
    %5 = llvm.load %4 {alignment = 4 : i64} : !llvm.ptr -> i32 // this should be 0
    // try memset as well

    // set it to 0x01010101
    %trunced = llvm.trunc %1 : i32 to i8
    "llvm.intr.memset"(%3, %trunced, %len, %isvolatile) : (!llvm.ptr, i8, i32, i1) -> ()
    // sub 0x01010101
